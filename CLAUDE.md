...
`social_links` is a JSON array of `{label, url, icon, is_visible}` on the profile, edited under **Edit page → Social links**. Because it is a column and not a child table, the `is_visible` filtering that `payload()` applies to the child collections does not reach it — `withVisibleSocialLinks()` handles it separately, on a clone, so revision snapshots still keep every link. The public page renders them twice: in the desktop side rail, and in the middle of the page footer between the location and availability notes. That footer is a three-track grid rather than `justify-between`, so the links sit in the true centre whatever length those two notes are, and `.site-footer-spacer` holds the middle track open when there are none. Both places drop out when nothing is visible. `icon` resolves through `iconMap`, so `AppIconSelect` is the editor rather than a free-text field.
