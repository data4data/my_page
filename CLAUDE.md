...
`social_links` is a JSON array of `{label, url, icon, in_rail, in_footer}` on the profile, edited under **Edit page → Social links**. **The two placements are independent** — a link can sit in the side rail, in the page footer, in both, or in neither — so each place draws its own set and each disappears on its own when nothing wants it. `is_visible` is the single switch they replaced; a link saved under it carries no placement keys, so both `PortfolioContentService::showsIn()` and `showsIn()` in `resources/js/shared/portfolio.js` fall back to it. Reading a missing placement as "off" would have emptied the rail and the footer at once on every install that already had links. **Those two functions mirror each other — keep them in step.** Because it is a column and not a child table, the `is_visible` filtering that `payload()` applies to the child collections does not reach it — `withVisibleSocialLinks()` handles it separately, on a clone, so revision snapshots still keep every link. The public page renders them twice: in the desktop side rail, and in the middle of the page footer between its two free-text notes (`location_note` and `availability_note`, the latter named for what it used to hold). That footer is a three-track grid rather than `justify-between`, so the links sit in the true centre whatever length those two notes are, and `.site-footer-spacer` holds the middle track open when there are none. `icon` resolves through `iconMap`, so `AppIconSelect` is the editor rather than a free-text field.
...
`SetPublicLocale` middleware puts the request into the URL's language so Laravel's own validator answers in it — `lang/nl/validation.php` holds only the rules the connect form can trip, and everything else falls back to `en`.
...
- `i18n.js` — `lang`/`copy`/`t`, and the merge point for the `ui` EN/NL dictionary. The dictionary is **split by audience** the same way `pages/` is: `i18n-public.js` for the visit card and the connect modal, `i18n-admin.js` for the workspace, and the handful of strings both halves use left in `i18n.js` itself. All three are merged into one `ui`, so `copy()` and `t()` are unchanged and no component knows about the split — a key in the "wrong" file still resolves. `lang` is a module-level singleton persisted to `localStorage`. **`copy()` falls back to English on a missing key**, so a dropped Dutch string ships silently; `i18n.test.js` asserts `en` and `nl` hold the same keys and that none is empty.
...
`forms.css` is shared by both halves and is itself written against the surface tokens, so a field themes itself wherever it is rendered — including the Select and DatePicker panels, which portal to `<body>` and so sit outside `.admin-shell` but still inside the `:root` that carries `color-scheme`. The public connect modal is the one place that must *not* follow the workspace, and `.public-modal` in `overlays.css` remaps those tokens back to the brand palette. That remap used to be keyed to `:root:not([data-theme])` — "not in the workspace" — because `TaskModal` and `CategoryModal` shared the same card class and had to be told apart from it. They are a different component now, so the remap sits on the card itself and needs no condition.
