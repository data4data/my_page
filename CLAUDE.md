...
- `i18n.js` — `lang`/`copy`/`t`, the strings both halves use, and `registerUi()`. The dictionary is **split by audience** the same way `pages/` is: `i18n-public.js` for the visit card and the connect modal, `i18n-admin.js` for the workspace, and the handful both use left in `i18n.js`.
